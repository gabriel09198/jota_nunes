...
const toFieldName = (label: string) =>
  label.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9\s]/gi, "").trim().split(/\s+/)
...
const formSections = secoes.map((secao) => ({
  titulo: secao.titulo,
  ambientes: secao.subtitulos.map((nome) => ({ label: nome, name: toFieldName(nome) })),
}));
...
const FormField = ({ ambiente, control, quartoChecked, sanitarioChecked }: any) => {
  const renderQuantity = (name: keyof CasaForm, label: string) => (
    <div className="mt-4">
      <label className="block text-gray-700 font-medium mb-1">Informe a Quantidade:</label>
      <Controller
        name={name}
        control={control}
        render={({ field }) => (
        ...
        <div>
          <label className="flex items-center justify-between border rounded-xl p-4 cursor-pointer has-[:checked]:bg-red-50 has-[:checked]:border-red-600">
            ...
            <div className="relative h-6 w-6">
              <input type="checkbox" className="peer sr-only" checked={!!field.value} onChange={(e) => field.onChange(e.target.checked)} ref={field.ref} />
          ...
          {ambiente.name === "quartoESuite" && quartoChecked && renderQuantity("quartoESuiteQuantidade", "Quarto e Suíte")}
          {ambiente.name === "sanitarioLavabo" && sanitarioChecked && renderQuantity("sanitarioLavaboQuantidade", "Sanitário/ Lavabo")}
        </div>
      )}
    />
  );
...
export default function StepTwoPage() {
  ...
  const { control, handleSubmit, watch, setValue, getValues } = useForm<CasaForm>({
      ...
      <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <div className="text-center mb-10">
            ...
            <p className="text-gray-500 mt-2">Selecione todos os ambientes que a obra possui.</p>
          </div>

          <form onSubmit={handleSubmit(onSubmit)} className="space-y-12">
            {formSections.map((secao) => (
              <section key={secao.titulo}>
                <h2 className="text-2xl font-bold text-red-700 mb-6 border-b-2 border-red-200 pb-2">{secao.titulo}</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-start">
                  {secao.ambientes.map((ambiente) => (
                    <FormField key={ambiente.name} ambiente={ambiente} control={control} quartoChecked={quartoChecked} sanitarioChecked={sanitarioChecked} />
                  ))}
                </div>
              </section>
            ))}

            <button type="submit" className="cursor-pointer w-full bg-red-600 hover:bg-red-700 text-white text-lg font-bold py-4 px-8 rounded-xl shadow-lg hover:shadow-red-500/50 transition-transform hover:-translate-y-1">
              Avançar
            </button>
          </form>
        </div>
